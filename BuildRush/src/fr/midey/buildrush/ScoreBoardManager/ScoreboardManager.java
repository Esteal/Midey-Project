...
public class ScoreboardManager {
    ...
    public ScoreboardManager() {
        ...
        glowingTask = BuildRush.getInstance().getScheduledExecutorService().scheduleAtFixedRate(() ->
        {
            String ip = colorIpAt();
            for (PersonalScoreboard scoreboard : scoreboards.values())
                BuildRush.getInstance().getExecutorMonoThread().execute(() -> scoreboard.setLines(ip));
        }, 80, 80, TimeUnit.MILLISECONDS);
 
        reloadingTask = BuildRush.getInstance().getScheduledExecutorService().scheduleAtFixedRate(() ->
        {
            for (PersonalScoreboard scoreboard : scoreboards.values())
            	BuildRush.getInstance().getExecutorMonoThread().execute(scoreboard::reloadData);
